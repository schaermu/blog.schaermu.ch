...
test.describe('blog post and series', () => {
  test('welcome post renders metadata, hero image, prose, and series badge', async ({ page }) => {
    ...
    await expect(page.locator('main a[href="/tags/e2e-testing/"]')).toBeVisible();
    await expect(page.locator('main a[href="/tags/e2e-astro/"]')).toBeVisible();
    await expect(page.locator('main a[href="/tags/e2e-web-dev/"]')).toBeVisible();
  ...
  test('post without hero image omits hero section', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1 })).toHaveText('E2E Fixture Post Without Hero');
    await expect(page.locator('img[alt="E2E Fixture Post Without Hero"]')).toHaveCount(0);
    await expect(page.locator('.prose')).toBeVisible();
  });
  ...
  test('mobile series quick navigation expands and shows links', async ({ page, isMobile }) => {
    ...
    await expect(mobileDetails.locator('a[href="/blog/e2e-fixture-no-hero/"]')).toBeVisible();
    ...
    await expect(secondMobileDetails.locator('a[href="/blog/e2e-fixture-with-hero/"]')).toBeVisible();
  });
