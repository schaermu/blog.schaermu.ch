...
test.describe('tags', () => {
  test('tags index shows heading and all fixture tag links with # prefix', async ({
    page,
  }) => {
    ...
    await expect(page.locator('a[href="/tags/e2e-testing/"]')).toContainText(
      '#e2e-testing',
    );
  });

  test('tag page for shared tag shows both fixture posts with time elements', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('a[href="/blog/e2e-fixture-no-hero/"]'),
    ).toBeVisible();

    await expect(page.locator('article time').first()).toBeVisible();

    const withHeroTop = await page
      .locator('a[href="/blog/e2e-fixture-with-hero/"]')
      .evaluate(
        (el) =>
          (el.closest('article') as HTMLElement).getBoundingClientRect().top,
      );
    const noHeroTop = await page
      .locator('a[href="/blog/e2e-fixture-no-hero/"]')
      .evaluate(
        (el) =>
          (el.closest('article') as HTMLElement).getBoundingClientRect().top,
      );
  ...
  test('tag page for unique tag shows only the fixture post that has that tag', async ({
    page,
  }) => {
  ...
  test('clicking a tag page post link navigates to the fixture blog post', async ({
    page,
  }) => {
    await page.goto('/tags/e2e-testing/');
